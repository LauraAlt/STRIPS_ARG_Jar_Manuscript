...
mod_ermF <- lm(log(ermF_rel) ~ Day + Vegetation + Site + Day:Vegetation + Day:Site + Site:Vegetation + Day:Site:Vegetation,
               data = data)

em <- emtrends(mod_ermF, pairwise ~ Vegetation | Site, var = "Day")

tiff(file = "ermF.tiff", width = 7, height = 4, units = "in", res = 500)
